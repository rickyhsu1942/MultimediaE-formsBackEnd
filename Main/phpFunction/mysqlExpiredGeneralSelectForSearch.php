<?php
	...
	if (isset($_GET['sqlCommandLentKind'])) {
		...
		$sqlCommands = " WHERE LEFT(general.formId,1) = '".$sqlCommandLentKind."' AND DATE_ADD(lendDate,INTERVAL deadLine DAY) < CURDATE() AND (returnUndertaker is null OR returnUndertaker='') ";
	...
	$sql = "SELECT * FROM general LEFT JOIN lent ON general.formId = lent.formId ".$sqlCommands;
	//加上限制顯示SQL
	...
	while($row = mysql_fetch_array($sqlResult)) {
		$jsonProperty[] = array("formId" => $row["formId"], "department" => urlencode($row["department"]), "credentials" => urlencode($row["credentials"])
			, "studentId" => urlencode($row["studentId"]), "name" => urlencode($row["name"]), "phoneNumber" => $row["phoneNumber"]
