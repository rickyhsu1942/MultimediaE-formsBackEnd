...
<script type="text/javascript">
	...
	var loadSqlEquipmentData = function(CommandKey,CommandValue) {
		$.ajax({
			...
			if (property.length > 0) {
				tableHtml += "<table align='center' border=1 style='width:37%'>" + 
							 ...
							 "<tr><th align='right'>商品名稱：</th><td align='left'><input type='textfield' id='equipmentNumber' value='" + property[0].equipmentNumber + "' /></td></tr>" +
							 "<tr><th align='right'>財產編號：</th><td align='left'><input disabled style='border:0px;background:transparent;' type='textfield' id='propertyNumber' value='" + property[0].propertyNumber + "' /></td></tr>" +
							 "<tr><th align='right'>狀　　態：</th><td align='left'><select id='stateSelect'>";
				for (var index in propertyState) {
					if (index == property[0].equipmentState - 1) {
						tableHtml += "<option selected>" + propertyState[index].stateName + "</option>"
					} else {
				...
				tableHtml += "</select></td></tr>" +
							 ...
							 "<tr><th align='right'>保存年限(月)：</th><td align='left'><input type='textfield' id='deadLine' value='" + property[0].deadLine + "' />個月</td></tr>" +
							 "<tr><th align='right'>保管人：</th><td align='left'><input type='textfield' id='custodian' value='" + property[0].custodian + "' /></td></tr>" +
							 ...
							 "<tr><td align='center' colspan='2'><input type='button' value='確認' onclick='UpdateSqlEquipmentData()' /></td></tr></table>";
			}
	...
	//修改MySQL器材型錄資料
	var UpdateSqlEquipmentData = function(){
		...
		$.ajax({
			type:"POST",
			url:"phpfunction/mysqlEquipmentUpdate.php",
			data:{sqlCommandUpdateData:updateJson}
		}).done(function(data) {
			console.log(data);
			if (data == 1) {
				location.href = 'equipmentUpdateDelete.php'
				alert("修改成功");
			} else {
				alert("修改失敗" + data);
			}
 ...
 <div class="main">
	...
	<div class="details">
		<div id="contentTitle" align="center">
			<font style="font-size:20px; font-weight:bold;">器材型錄表單 - 修改<font>
		</div><br>
		<div id="content"></div>
 	</div>
	<div class="clear"></div>
