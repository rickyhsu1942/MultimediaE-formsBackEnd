...
<script type="text/javascript">
	...
	$(document).on("ready",function() {
		loadSqlEquipmentData(1,"equipmentNumber");
	});
	...
	var loadSqlEquipmentData = function(page,CommandOrder,CommandKey,CommandValue) {
		$.ajax({
			type:"POST",
			url:"phpfunction/mysqlOtherEquipmentSelectForSearch.php",
			...
			var tableHtml = "";
			tableHtml = "<table border=1 style='width:100%'><tr><th>勾選</th><th>器材編號</th><th>器材數量</th><th>借出數量</th><th>剩餘數量</th><th>存放地點</th><th>功能</th></tr>";
			for (var index in property) {
				number ++;
				totalPages = property[index].totalPages;
				totalRows = property[index].totalRows;
				tableHtml += "<tr><td align=center><input type='checkbox' name='checkboxName' id='checkbox" + index + "'/></td><td>" +  property[index].equipmentNumber + "</td><td align='center'>" + property[index].equipmentQuantity + 
							 "</td><td align='center'>" + property[index].lentQuantity + "</td><td align='center'>" + (property[index].equipmentQuantity - property[index].lentQuantity) + 
							 "</td><td>" + property[index].location + "</td><td align='center'><form action='otherEquipmentUpdate.php' method='post'>" +
							 "<input type='submit' value='修改'/><input type='hidden' name='sqlEquipmentNumber' value='" + property[index].equipmentNumber + "' /></form></td></tr>";
			}
			tableHtml += "</table>"
			//頁數Table
	...
	function deleteClicked() {
		...
		for (var i = 0; i < checkboxs.length; i++) {
			if (checkboxs[i].checked) {
				deleteSqlOtherEquipmentData(property[i].equipmentNumber);
			}
		}
		alert("刪除完畢");
		searchClicked(1);
	...
	//刪除MySQL
	var deleteSqlOtherEquipmentData = function(equipmentNumber) {
		$.ajax({
			type:"POST",
			url:"phpfunction/mysqlOtherEquipmentDelete.php",
...
<div class="wrap">
 <div class="main">
	...
	<div class="details">
		...
		<font style="font-size:20px; font-weight:bold;">無財產編號器材型錄表單 - 修改/刪除<font>
		</div><br>
		<div id="search">
				...
				<tr>
					<td align="left">
						...
						<select id="orderSelect" onchange="searchClicked(1)">
							...
							<option id="equipmentQuantity">器材數量</option>
							<option id="lentQuantity">借出數量</option>
							<option id="location">存放地點</option>
						</select>
						<font>排序</font>
					</td>
					<td align="right">
