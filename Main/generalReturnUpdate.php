...
<script type="text/javascript">
	//全域變數
	var general;
	var generalCredentials = ["學生證","身分證","健保卡","駕照","其他"];
	...
	$(document).on("ready",function() {
		loadSqlGeneralData("general.formId",<?php print("\"".$formId."\""); ?>);
	});

	//載入MySQL器材型錄資料
	var loadSqlGeneralData = function(CommandKey,CommandValue) {
		$.ajax({
			type:"GET",
			url:"phpfunction/mysqlGeneralSelect.php",
			...
			var tableHtml = "";
			if (general.length > 0) {
				document.getElementById("department").value = general[0].department;
				for (var index in generalCredentials) {
					var credentialsOption = document.createElement("option");
					credentialsOption.text = generalCredentials[index];
					if (generalCredentials[index] == general[0].credentials) {
						credentialsOption.selected = "selected";
					}
					document.getElementById("credentialsSelect").add(credentialsOption);
				}
				document.getElementById("studentId").value = general[0].studentId;
				document.getElementById("name").value = general[0].name;
				document.getElementById("phoneNumber").value = general[0].phoneNumber;
				document.getElementById("deadline").value = general[0].deadline;
				document.getElementById("emailAddress").value = general[0].emailAddress;
				document.getElementById("lendDate").value = general[0].lendDate;
				document.getElementById("lendUndertaker").value = general[0].lendUndertaker;
				...
				for (var index in general) {
					...
					equipmentNumberContentText.value = general[index].equipmentNumber;
					...
					lentIdHidden.value = general[index].lentId;
					...
					equipmentQuantityContentText.value = general[index].equipmentQuantity;
					...
					returnDateContentText.value = general[index].returnDate;
					...
					returnUndertakerContentText.value = general[index].returnUndertaker;
				...
				submitButton.onclick = function(){UpdateSqlPropertyDataAndGeneral()};
	...
	//修改MySQL器材型錄資料
	var UpdateSqlPropertyDataAndGeneral = function(){
		...
		for (var index in general) {
			if (document.getElementById("equipmentQuantity" + index).value > general[index].equipmentQuantity){
				alert("裝置歸還數量錯誤!!");
				return;
			}
		}
		...
		for (var index in general) {
			...
			if ($('#returnCheckbox' + index)[0].checked) {
				...
				equipmentNumbers.lentId = general[index].lentId;
		...
		$.ajax({
			...
			console.log(data);
			if (data == 1) {
				location.href = 'generalUpdateDelete.php'
				alert("歸還修改成功");
			} else {
				alert("歸還修改失敗" + data);
			}
 ...
 <div class="main">
	...
	<div class="details">
		<div id="contentTitle" align="center">
			<font style="font-size:20px; font-weight:bold;">逾期未歸還之器材表單(一般借用表單) - 歸還/修改<font>
		</div><br>
		<div id="content">
			<table align='center' id="contentTable" border=1 style='width:60%'> 
				<tr><th align='center' colspan='4'><?php print($formId); ?></th></tr>
				<tr><th align='right'>系　　所：</th><td align='left'><input disabled type='textfield' id='department' value='' style='border:0px;background:transparent;' /></td>
					<th align='right'>抵押證件：</th><td align='left'><select disabled id='credentialsSelect'></select></td></tr>
				<tr><th align='right'>學　　號：</th><td align='left'><input disabled type='textfield' id='studentId' value='' style='border:0px;background:transparent;' /></td>
				<th align='right'>姓　　名：</th><td align='left'><input disabled type='textfield' id='name' value='' style='border:0px;background:transparent;' /></td></tr>
				...
				<th align='right'>借用期限：</th><td align='left'><input disabled type='textfield' id='deadline' value='' style='border:0px;background:transparent;' />天</td></tr>
				<tr><th align='right'>EMAIL：</th><td align='left' colspan='3'><input disabled type='textfield' id='emailAddress' value='' style='border:0px;background:transparent;' /></td></tr>
				<tr><th align='right'>出借日期：</th><td align='left'><input disabled type='textfield' readOnly=true id='lendDate' value='' style='border:0px;background:transparent;' /></td>
				<th align='right'>出借承辦人：</th><td align='left'><input disabled type='textfield' id='lendUndertaker' value='' style='border:0px;background:transparent;' /></td></tr>
			</table>
		</div>
 	</div>
	<div class="clear"></div>
