...
<script type="text/javascript">
	...
	$(document).on("ready",function() {
		loadSqlOtherEquipmentData("equipmentNumber",<?php print("\"".$equipmentNumber."\""); ?>);
	});

	//載入MySQL器材型錄資料
	var loadSqlOtherEquipmentData = function(CommandKey,CommandValue) {
		$.ajax({
			type:"POST",
		...
		$.ajax({
			...
			if (property.length > 0) {
				...
				for (var index in equipmentLentRecord) {
					...
					tableHtml += "<tr><td align='left'><input disabled style='border:0px;background:transparent;' type='textfield' id='equipmentNumber' value='" + equipmentLentRecord[index].equipmentNumber + " * " + equipmentLentRecord[index].equipmentQuantity + "'  /></td>" + 
