...
<script type="text/javascript">
	...
	var loadSqlEquipmentData = function(page,CommandOrder,CommandKey,CommandValue) {
		$.ajax({
			...
			var tableHtml = "";
			tableHtml = "<table border=1 style='width:100%'><tr><th>序號</th><th>器材編號</th><th>器材數量</th><th>借出數量</th><th>剩餘數量</th><th>存放地點</th><th>歷史紀錄</th></tr>";
			for (var index in property) {
				number ++;
				totalPages = property[index].totalPages;
				totalRows = property[index].totalRows;
				tableHtml += "<tr><td>" + number + "." + "</td><td>" +  property[index].equipmentNumber + "</td><td align='center'>" + property[index].equipmentQuantity + 
							 ...
							 "</td><td>" + property[index].location + "</td>" +
							 "<td align='center'><form action='otherEquipmentRecordSelect.php' method='post'><input type='submit' value='查詢' />" +
...
<div class="wrap">
 <div class="main">
	...
	<div class="details">
		...
		<font style="font-size:20px; font-weight:bold;">無財產編號器材型錄表單 - 查詢<font>
		</div><br>
		<div id="search">
